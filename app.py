...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get(
    'DATABASE_URL',
    'mysql+mysqlconnector://root:EfzXhkBXljeGiPDXodhgPCHuLEqWsvme@mysql.railway.internal:3306/railway'
)
...
app.secret_key = os.environ.get('SECRET_KEY', 'burgerhut_secret')
...
def logout():
    ...
    session.clear()   # 🔥 badhu session clean
...
def my_orders():
    ...
    if from_date and to_date:
        ...
        # 🔥 important fix: include full day range
...
def admin_add_menu():
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            file.save(filepath)
            image_path = f"/static/images/menu/{filename}"  # relative path
...
def admin_order_details(order_id):
    ...
    for item in items:
        ...
        detailed_items.append({
            "name": menu_item.item_name if menu_item else "Item Deleted",
            "quantity": item.quantity,
            "price": item.price
        })
...
def delivery_dashboard():
    ...
    for order in delivered_orders:
        if order.order_date.month == current_month and order.order_date.year == current_year:
            monthly_income += order.total_amount * 0.20   # 🔥 20% commission
...
@app.route("/delivery_order_details/<int:order_id>")
